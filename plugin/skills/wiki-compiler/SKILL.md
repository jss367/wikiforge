...
- **Every individual decision record** (`decisions/*.md`, `ADR-*.md`, `docs/adr/*.md` — one sub-page per decision, even short ones; use the file basename without the `.md` extension as the sub-slug, regardless of whether the basename is dated, numbered, or named). If two decision files would produce the same sub-slug (e.g. `decisions/auth.md` and `docs/adr/auth.md`), disambiguate by prefixing the immediate parent directory name: `decisions-auth` and `adr-auth`. Decisions are the load-bearing why-records of the project and must remain individually addressable, and no decision may be silently overwritten by another.
