...
- `topics.<slug>.compiler_hash`: SHA-256 (first 16 hex chars) of the files that determine compiler behavior when *this topic* was last compiled. Hash over every file matching these globs under `${CLAUDE_PLUGIN_ROOT}`, in sorted path order:
  - `skills/wiki-compiler/SKILL.md` — compilation algorithm
  - `templates/*.md` — article/sub-article/schema templates
  - `commands/wiki-compile.md` — orchestrator prompt
  Recipe: `{ for f in <sorted paths>; do shasum -a 256 "$f"; done; } | shasum -a 256 | cut -c1-16`. Any edit to compiler logic, templates, or the compile orchestrator changes this hash automatically — no version bump required.

The `compiler_hash` is stored **per topic**, not globally. A topic that didn't recompile in the last run retains its old hash, so the next compile still detects the mismatch for that specific topic — the signal survives scoped `--topic` runs.
...
3. **Compiler change?** Compute the current `compiler_hash`. If it differs from this topic's stored `compiler_hash` → RECOMPILE. This is what propagates heuristic changes retroactively: a topic compiled under old rules keeps its old stored hash until it's re-evaluated under the new ones.
...
- If the compiler-change check (step 3) fired for every topic and drove a full rebuild (i.e. no `--topic` was passed and the `compiler_hash` differed from every stored entry), surface it as: "Compiler logic changed since last compile — full rebuild." This is the normal case when a user edits the skill, templates, or compile orchestrator and runs `/wiki-compile` next.
- If `--topic <slug>` was passed and the compiler changed, recompile only the targeted topic and warn: "Compiler logic changed since last compile, but `--topic` was passed — other topics remain on old rules. Run `/wiki-compile` (without `--topic`) to propagate the change." Do not touch other topics' stored values.
...
- Edits to `SKILL.md`, any file under `templates/`, or `commands/wiki-compile.md` change the `compiler_hash` automatically and invalidate each topic the next time it's evaluated via step 3 above. This propagates heuristic changes retroactively with no manual bookkeeping. If a future change adds a new file that should influence compilation, update the `compiler_hash` recipe above to include it.

**Persisting the state (in Phase 5):**

After each topic is compiled, update `.compile-state.json`. Only update a topic's `compiler_hash` when *that topic* actually recompiled — skipped topics keep their previous hash, which is what preserves the compiler-change signal across scoped `--topic` runs.
```json
{
  "topics": {
    "gradient-routing": {
      ...
      "config_hash": "<hash>",
      "compiler_hash": "<first 16 hex chars of SHA-256>",
...
```json
{
  ...
  "topics": {
    "{slug1}": {
      ...
      "compiler_hash": "{first 16 hex chars of SHA-256 over SKILL.md + templates/*.md + commands/wiki-compile.md, sorted}",
...
For topics that were skipped in this run, preserve their existing entry unchanged — including their `compiler_hash`, which is what makes the compiler-change signal survive a `--topic` scoped run. For topics that recompiled, update `sources`, `config_hash`, `compiler_hash`, and `last_compiled` to the current values. For topics no longer present (removed from schema by user), remove their entry.

**Migrating from older state:** Entries written by earlier versions of this skill used `plugin_version` + `skill_hash` instead of `compiler_hash`. Treat any topic whose entry lacks `compiler_hash` (regardless of whether the legacy fields are present) as failing the compiler-change check in step 3 above — i.e. RECOMPILE it. Drop the legacy fields when writing the updated entry.
