...
2. **Resolve wikilinks** (`![[name]]`) against the vault root, not the current file's directory. Obsidian's default resolution is "shortest path from vault root that uniquely matches the name" — for v1, do a simple recursive glob for the filename under the folder; if multiple candidates, prefer one in an `attachments/` or `images/` subfolder; if still ambiguous, log a warning and use the first match.
