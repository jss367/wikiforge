...
1. **Recognise three syntaxes, including Obsidian's display modifiers:**
   - `![[name.png]]`, `![[name.png|alias]]`, `![[name.png|300]]`, `![[name.png|alias|300]]` — Obsidian wikilink embeds. Anything after the first `|` is a display modifier: a bare integer is a width hint (ignored for v1 — Google Docs sizes images intrinsically), anything else is alt text. The image target is always the substring before the first `|`.
